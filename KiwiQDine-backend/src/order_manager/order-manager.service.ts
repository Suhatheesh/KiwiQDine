...
export class OrderManagerService extends AuthService {
  ...
  constructor(
    ...
    private readonly validateOrderManager: IValidateUser<OrderManager, OrderManagerEntity>,
  ...
  async createOrderManager(props: CreateOrderManager): Promise<IOrderManagerDTO> {
    ...
    const existingOrderManager = await this.orderManagerRepository.findOne({ where: { email: normalizedEmail } });
    if (existingOrderManager.isSuccess) {
      throwApplicationError(HttpStatus.BAD_REQUEST, 'Order Manager already exists');
    }
    const validateSingleClient = await this.singleclientRepository.findById(props.singleclientId);
  ...
  async getOrderManager(id: string): Promise<IOrderManagerDTO> {
