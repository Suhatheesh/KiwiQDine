...
import { QRTableType, QRStatus, TenantType, OrderType } from "../../utils/constants";
...
import { Plus, QrCode, Trash, Image as ImageIcon, Download } from "lucide-react";
...
const QRTab: FC<QRTabProps> = ({ restaurantId }) => {

    const { tenantId, type } = useParams<{ id: string, tenantId: string, type: string }>();
    ...
    const { canCreateQR, canCreateTable } = useSelector((state: RootState) => state.subscriptions);

    const { register, handleSubmit, reset, watch } = useForm<CreateQRRquest>({
        defaultValues: {
            name: "",
            description: "",
            type: type === TenantType.FOOD_COURT ? QRTableType.FOOD_COURT : QRTableType.TAKE_AWAY
        }
    });
    ...
    useLayoutEffect(() => {
        if (type === TenantType.FOOD_COURT && restaurantId) {
            dispatch(fetchCanQRCreateRequest(restaurantId))
            dispatch(fetchAllQRRequest({ tenantId: tenantId, page: 1, limit: 10 }));
        } else {
            if (restaurantId) {
                dispatch(fetchCanCreateTableRequest(restaurantId))
                dispatch(fetchAllQRRequest({ restaurantId: restaurantId, page: 1, limit: 10 }));
            }
        }
    }, [dispatch])
    ...
    const handleCreateUserRole: SubmitHandler<CreateQRRquest> = (data) => {
        dispatch(createQRRequest({ ...data, restaurantId: restaurantId }))
    }
    ...
    const filterQRList = useMemo(() => {
        if (type === TenantType.RESTAURANT) {
            return qr.filter((i) => i.type === QRTableType.TAKE_AWAY && (i.restaurant && i.restaurant.tenantId) === tenantId)
        }
        return qr.filter((i) => i.type === QRTableType.FOOD_COURT && (i.restaurant && i.restaurant.tenantId) === tenantId)
    }, [qr, tenantId])
    ...
    const getQRCodeValue = () => {
        if (!selectQR) return '';
        if (selectQR.type === QRTableType.TAKE_AWAY && type === TenantType.RESTAURANT) {
            return `${import.meta.env.VITE_CUSTOMER_QR_BASE_URL}/restaurant/qr/${tenantId}/${restaurantId}/${selectQR?.id}/${selectQR?.type.includes(QRTableType.TAKE_AWAY) ? OrderType.TAKEAWAY : OrderType.PARKING}`;
        }
        return `${import.meta.env.VITE_CUSTOMER_QR_BASE_URL}/foodcourt/qr/${tenantId}/${selectQR?.id}/${selectQR?.type.includes(QRTableType.FOOD_COURT) ? OrderType.DINEIN : OrderType.PARKING}`;
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex-1 flex md:flex-row flex-col items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 mb-1">{type === TenantType.FOOD_COURT ? "QR Code Management" : "QR Code Management"}</h1>
                    <p className="text-sm text-gray-500">Create and manage scannable QR codes for {type === TenantType.FOOD_COURT ? "food court" : "takeaway"} orders</p>
                </div>
                <Button
                    className="w-full md:w-auto md:mt-0 mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white border-0 shadow-lg shadow-blue-200/50 font-semibold"
            ...
            <Modal
                ...
                footer={
                    <div className="flex items-center justify-end gap-3">
                        ...
                        <Button onClick={handleSubmit(handleCreateUserRole)} isLoading={loading} disabled={handleCreateButtonDisabled()}>
                            Create QR
                        </Button>
                    </div>
                }
                ...
                <form>
                    <div className="space-y-3">
                        <Select
                            label="Type"
                            options={[
                                ...(type === TenantType.RESTAURANT ? [{ value: QRTableType.TAKE_AWAY, label: "Take away" }] : [{ value: QRTableType.FOOD_COURT, label: "Food court" }]),
                        ...
                        {!showDownloadOptions ? (
                            <div className="flex items-center gap-3 w-full">
                                ...
                                <Button
                                    ...
                                    isLoading={loading}
                                    className="flex-1 justify-center bg-gradient-to-r from-red-500 to-orange-600 hover:from-red-600 hover:to-orange-700 text-white border-0"
                                >
                ...
                <div className="space-y-6">
                    ...
                    <div className="relative">
                        ...
                        <div className="relative bg-white rounded-2xl p-8 shadow-xl">
                            ...
                            <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2 rounded-tl-lg"></div>
                            <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2 rounded-tr-lg"></div>
                            <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2 rounded-bl-lg"></div>
                            <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2 rounded-br-lg"></div>
                    ...
                    <div className="text-center space-y-2 p-6 rounded-xl w-full border">
