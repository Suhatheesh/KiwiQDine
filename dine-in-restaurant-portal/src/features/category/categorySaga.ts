...
function* fetchAllCategorySaga(): Generator<CallEffect | PutEffect, void, CategoryResponse> {
    try {
        const response = yield call(CategoryAPI.fetchAllCategory);
...
function* deleteCategorySaga({ payload: args }: PayloadAction<string[]>): Generator<CallEffect | PutEffect, void, any> {
    try {
        yield call(CategoryAPI.deleteCategory, args);
        yield putResolve(deleteCategorySuccess(args));
    } catch (error) {
        toast.error(handleApiError(error));
...
function* reorderCategoriesSaga({ payload: args }: PayloadAction<{ restaurantId: string, categories: ReorderCategoriesRequest }>): Generator<CallEffect | PutEffect, void, boolean> {
    try {
        yield call(CategoryAPI.reorderCategories, args);
        toast.success(`Category order updated successfully!`)
        yield putResolve(reorderCategoriesSuccess(true));
        yield put(fetchAllCategoryRequest());
    } catch (error) {
        toast.error(handleApiError(error));
        yield put(reorderCategoriesFaliure(handleApiError(error)))
    }
}
...
export function* watchCategorySaga() {
    ...
    yield takeLatest(reorderCategoriesRequest.type, reorderCategoriesSaga);
}